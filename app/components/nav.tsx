      ...
      <main className="flex flex-col lg:hidden w-full bg-nav bg-no-repeat bg-cover relative z-20">
        ...
        >
          ...
          <div className="flex gap-2 sm:gap-4 items-center">
            <button
              className={`bg-jugz transition-all w-[100px] sm:w-[120px] h-[33px] flex justify-center items-center rounded-lg text-white text-[12px] sm:text-[14px] font-medium hover:scale-[1.02] ${roboto.className}`}
            >
              Connect Wallet
            </button>
            ...
            <div className="flex flex-col w-full self-center pt-5 gap-1">
              {routes.map((item, index) => (
                <Link
                  ...
                  href={item.href}
                  className={`
                    transition-all font-light text-[16px] bg-jugz p-4 bg-opacity-90 hover:bg-opacity-60 hover:text-white hover:border-l-[6px] hover:border-white border-purple-betz rounded-lg shadow-md                     
                    ${
                      item.href === page.href &&
